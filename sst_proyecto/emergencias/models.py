...
class Emergencia(models.Model):
    ...
    # Datos basicos
    tipo = models.ForeignKey(TipoEmergencia, on_delete=models.PROTECT, related_name="emergencias", db_index=True)
    reportada_por = models.ForeignKey(
        Usuario, on_delete=models.SET_NULL, null=True, related_name="emergencias_reportadas", db_index=True
    )
    ...
    edificio = models.ForeignKey(
        EdificioBloque, on_delete=models.SET_NULL, null=True, blank=True, related_name="emergencias"
    )
    ...
    # Respuesta
    atendida_por = models.ManyToManyField(Usuario, related_name="emergencias_atendidas", blank=True)
    ...
    marcada_falsa_por = models.ForeignKey(
        Usuario, on_delete=models.SET_NULL, null=True, blank=True, related_name="falsas_alarmas_marcadas"
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=["estado", "fecha_hora_reporte"], name="idx_emergencia_estado_fecha"),
        ]
    ...
    def esta_cerca_de_edificio(self, radio_metros=100):
        ...
        for edificio in edificios_cercanos:
            distancia = calcular_distancia(self.latitud, self.longitud, edificio.latitud, edificio.longitud)
...
class BrigadaEmergencia(models.Model):
    ...
    usuario = models.OneToOneField(Usuario, on_delete=models.CASCADE, related_name="brigada")
...
class NotificacionEmergencia(models.Model):
    ...
    emergencia = models.ForeignKey(Emergencia, on_delete=models.CASCADE, related_name="notificaciones")
...
class ContactoExterno(models.Model):
    ...
    tiempo_llegada_estimado = models.IntegerField(help_text="Minutos estimados de llegada", null=True, blank=True)
