...
class ReporteViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # INSTRUCTOR: Solo ve reportes de asistencia que generó
            ...
            return ReporteGenerado.objects.filter(
                Q(generado_por=user) | Q(configuracion__tipo_reporte="ASISTENCIA")
            ).order_by("-fecha_generacion")

        # VIGILANCIA: Solo reportes de aforo y seguridad
        elif user.rol == "VIGILANCIA":
            return ReporteGenerado.objects.filter(configuracion__tipo_reporte__in=["AFORO", "SEGURIDAD"]).order_by(
                "-fecha_generacion"
            )

        # BRIGADA: Solo reportes de incidentes/emergencias
        elif user.rol == "BRIGADA":
            return ReporteGenerado.objects.filter(configuracion__tipo_reporte__in=["INCIDENTES", "SEGURIDAD"]).order_by(
                "-fecha_generacion"
            )
    ...
    def incidentes(self, request):
        ...
        if request.user.rol == "INSTRUCTOR":
            datos = ReporteIncidentesService.generar_reporte_instruccion(periodo_inicio, periodo_fin, request.user)
        else:
        ...
        if formato == "pdf":
            ...
            response["Content-Disposition"] = (
                f'attachment; filename="reporte_incidentes_{fecha_inicio}_{fecha_fin}.pdf"'
            )
            return response
            ...
            response["Content-Disposition"] = (
                f'attachment; filename="reporte_incidentes_{fecha_inicio}_{fecha_fin}.xlsx"'
            )
            return response
            ...
            response = HttpResponse(csv_buffer.getvalue(), content_type="text/csv; charset=utf-8")
            response["Content-Disposition"] = (
                f'attachment; filename="reporte_incidentes_{fecha_inicio}_{fecha_fin}.csv"'
            )
            return response
    ...
    def asistencia(self, request):
        ...
        if not all([ficha, fecha_inicio, fecha_fin]):
            return Response(
                {"error": "ficha, fecha_inicio y fecha_fin son requeridos."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if formato == "pdf":
            ...
            response = HttpResponse(pdf_buffer.getvalue(), content_type="application/pdf")
            response["Content-Disposition"] = (
                f'attachment; filename="reporte_asistencia_ficha_{ficha}_{fecha_inicio}_{fecha_fin}.pdf"'
            )
            return response
            ...
            response["Content-Disposition"] = (
                f'attachment; filename="reporte_asistencia_ficha_{ficha}_{fecha_inicio}_{fecha_fin}.xlsx"'
            )
            return response
            ...
            response = HttpResponse(csv_buffer.getvalue(), content_type="text/csv; charset=utf-8")
            response["Content-Disposition"] = (
                f'attachment; filename="reporte_asistencia_ficha_{ficha}_{fecha_inicio}_{fecha_fin}.csv"'
            )
            return response
    ...
    def seguridad(self, request):
        ...
        if not fecha_inicio or not fecha_fin:
            ...
            try:
                ...
                return Response(
                    {"error": "Formato de fecha inválido. Use YYYY-MM-DD"}, status=status.HTTP_400_BAD_REQUEST
                )
        ...
        if formato == "pdf":
            ...
            response = HttpResponse(pdf_buffer.getvalue(), content_type="application/pdf")
            response["Content-Disposition"] = (
                f'attachment; filename="reporte_seguridad_{fecha_inicio_str}_{fecha_fin_str}.pdf"'
            )
            return response
            ...
            response["Content-Disposition"] = (
                f'attachment; filename="reporte_seguridad_{fecha_inicio_str}_{fecha_fin_str}.xlsx"'
            )
            return response
            ...
            response = HttpResponse(csv_buffer.getvalue(), content_type="text/csv; charset=utf-8")
            response["Content-Disposition"] = (
                f'attachment; filename="reporte_seguridad_{fecha_inicio_str}_{fecha_fin_str}.csv"'
            )
            return response
    ...
    def mi_asistencia(self, request):
        ...
        if request.user.rol != "APRENDIZ":
            return Response({"error": "Esta función es solo para aprendices."}, status=status.HTTP_403_FORBIDDEN)
        ...
        registros = RegistroAcceso.objects.filter(
            usuario=user, fecha_hora_ingreso__date__gte=fecha_inicio, fecha_hora_ingreso__date__lte=fecha_fin
...
def dashboard_estadisticas(request):
    ...
    personas_en_centro = RegistroAcceso.objects.filter(fecha_hora_egreso__isnull=True).count()
    ...
    # Emergencias activas
    emergencias_activas = Emergencia.objects.filter(Q(estado="REPORTADA") | Q(estado="EN_ATENCION")).count()
