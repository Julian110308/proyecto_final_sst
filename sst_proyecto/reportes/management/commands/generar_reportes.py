...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--fecha-inicio", type=str, help="Fecha de inicio (YYYY-MM-DD). Default: hace 30 días")
        parser.add_argument("--fecha-fin", type=str, help="Fecha de fin (YYYY-MM-DD). Default: hoy")
        parser.add_argument("--ficha", type=str, help="Número de ficha (requerido para reporte de asistencia)")
        parser.add_argument("--todos", action="store_true", help="Genera todos los reportes configurados como activos")
        parser.add_argument(
            "--enviar-email", action="store_true", help="Envía los reportes por email a los destinatarios configurados"
        )
        parser.add_argument("--guardar", action="store_true", help="Guarda el reporte en la base de datos")
        parser.add_argument(
            "--output-dir",
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        if options["todos"]:
            ...
            self.stdout.write(self.style.WARNING("Especifique --tipo o --todos para generar reportes"))
    ...
    def _generar_reportes_programados(self, options, fecha_inicio, fecha_fin, output_dir):
        ...
        if not configuraciones.exists():
            self.stdout.write(self.style.WARNING("No hay configuraciones de reportes activas"))
            return
        ...
        for config in configuraciones:
            ...
            if not self._debe_generar(config):
                self.stdout.write("  - Saltando (no es momento de generar)")
                continue
            ...
            if not tipo:
                self.stdout.write(self.style.WARNING(f"  - Tipo de reporte no soportado: {config.tipo_reporte}"))
                continue
    ...
    def _generar_reporte_individual(
        self, tipo, formato, fecha_inicio, fecha_fin, ficha, guardar, enviar_email, output_dir, config=None
    ):
        ...
        try:
            if tipo == "aforo":
                ...
                if not ficha:
                    self.stdout.write(self.style.WARNING("  - Se requiere --ficha para reportes de asistencia"))
                    return
