...
class ReporteAforoService:
    ...
    def generar_reporte(periodo_inicio, periodo_fin):
        ...
        registros = RegistroAcceso.objects.filter(fecha_hora_ingreso__range=[periodo_inicio, periodo_fin])

        total_ingresos = registros.count()

        # Aforo por dia (optimizado con TruncDate)
        aforo_diario = (
            registros.annotate(fecha=TruncDate("fecha_hora_ingreso"))
            .values("fecha")
        ...
        # Aforo por rol
        aforo_por_rol = (
            Usuario.objects.filter(accesos__fecha_hora_ingreso__range=[periodo_inicio, periodo_fin])
            .values("rol")
            .annotate(total=Count("accesos"))
            .order_by("-total")
        )
        ...
        hora_pico_data = (
            registros.annotate(hora=ExtractHour("fecha_hora_ingreso"))
            .values("hora")
            .annotate(total=Count("id"))
            .order_by("-total")
            .first()
...
class ReporteIncidentesService:
    ...
    def generar_reporte(periodo_inicio, periodo_fin):
        ...
        emergencias = Emergencia.objects.filter(fecha_hora_reporte__range=[periodo_inicio, periodo_fin])

        total_emergencias = emergencias.count()

        # Por tipo - usar tipo__nombre en lugar de tipo_nombre
        por_tipo = emergencias.values("tipo__nombre").annotate(total=Count("id")).order_by("-total")
        ...
        # Por estado
        por_estado = emergencias.values("estado").annotate(total=Count("id")).order_by("estado")
        ...
        # Personas afectadas
        total_afectados = emergencias.aggregate(total=Count("personas_afectadas"))["total"] or 0
    ...
    def generar_reporte_instruccion(periodo_inicio, periodo_fin, usuario):
        ...
        return {
            ...
            "evacuaciones_requeridas": evacuaciones,
            "porcentaje_resueltas": (
                round((emergencias_resueltas.count() / total_emergencias * 100), 2) if total_emergencias > 0 else 0
            ),
...
class ReporteAsistenciaService:
    ...
    def generar_reporte(ficha, periodo_inicio, periodo_fin):
        ...
        aprendices = Usuario.objects.filter(rol="APRENDIZ", ficha=ficha, activo=True)
        ...
        for aprendiz in aprendices:
            ...
            dias_asistio = (
                RegistroAcceso.objects.filter(usuario=aprendiz, fecha_hora_ingreso__range=[periodo_inicio, periodo_fin])
                .extra({"fecha": "date(fecha_hora_ingreso)"})
                .values("fecha")
                .distinct()
                .count()
            )
            ...
            registros = RegistroAcceso.objects.filter(  # noqa: F841
                usuario=aprendiz, fecha_hora_ingreso__range=[periodo_inicio, periodo_fin]
            )
        ...
        return {
            ...
            "promedio_asistencia": round(
                sum(a["porcentaje_asistencia"] for a in reporte_aprendices) / len(reporte_aprendices), 2
            )
            if reporte_aprendices
            else 0,
        }
...
class ReporteSeguridadService:
    ...
    def generar_reporte(periodo_inicio, periodo_fin):
        ...
        equipamiento_por_tipo = (
            EquipamientoSeguridad.objects.values("tipo")
            .annotate(
                total=Count("id"),
        ...
        emergencias_por_tipo = (
            Emergencia.objects.filter(fecha_hora_reporte__range=[periodo_inicio, periodo_fin])
            .values("tipo__nombre")
            .annotate(total=Count("id"))
            .order_by("-total")
        )
        ...
        emergencias_por_tipo_lista = [
            {"tipo_nombre": item["tipo__nombre"], "total": item["total"]} for item in emergencias_por_tipo
        ]

        # Tendencias mensuales (sin usar .extra() que está deprecado)
        tendencias = (
            Emergencia.objects.filter(fecha_hora_reporte__range=[periodo_inicio, periodo_fin])
            .annotate(mes=TruncMonth("fecha_hora_reporte"))
            .values("mes")
            .annotate(total=Count("id"))
            .order_by("mes")
        )
        ...
        tendencias_lista = [
            {"mes": item["mes"].strftime("%Y-%m") if item["mes"] else "", "total": item["total"]} for item in tendencias
        ]
        ...
        return {
            ...
            "porcentaje_operativo": round(
                (equipamiento_operativo / equipamiento_total * 100) if equipamiento_total > 0 else 0, 2
            ),
    ...
    def generar_reporte_vigilancia(periodo_inicio, periodo_fin):
        ...
        try:
            visitantes = Visitante.objects.filter(fecha_visita__range=[periodo_inicio, periodo_fin])
    ...
    def generar_reporte_emergencias(periodo_inicio, periodo_fin):
        ...
        emergencias_por_tipo = emergencias.values("tipo__nombre").annotate(total=Count("id")).order_by("-total")
        ...
        tendencias = (
            emergencias.annotate(mes=TruncMonth("fecha_hora_reporte"))
            .values("mes")
            .annotate(total=Count("id"))
            .order_by("mes")
        )
        ...
        return {
            ...
            "porcentaje_operativo": round(
                (
                    EquipamientoSeguridad.objects.filter(estado="OPERATIVO").count()
                    / EquipamientoSeguridad.objects.count()
                    * 100
                )
                if EquipamientoSeguridad.objects.count() > 0
                else 0,
                2,
            ),
            "equipamiento_por_tipo": list(
