...
def listar_incidentes(request):
    ...
    if fecha_desde:
        try:
            ...
            incidentes = incidentes.filter(fecha_reporte__date__gte=datetime.strptime(fecha_desde, "%Y-%m-%d").date())
        except ValueError:
            pass
    if fecha_hasta:
        try:
            ...
            incidentes = incidentes.filter(fecha_reporte__date__lte=datetime.strptime(fecha_hasta, "%Y-%m-%d").date())
        except ValueError:
            pass
    ...
    criticos_sla = (
        incidentes.exclude(estado__in=["RESUELTO", "CERRADO"])
...
def crear_incidente(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f'Incidente "{incidente.titulo}" reportado exitosamente. Se ha generado una alerta al personal correspondiente.',
            )
...
def exportar_incidentes_excel(request):
    ...
    if q:
        ...
        incidentes = incidentes.filter(Q(titulo__icontains=q) | Q(descripcion__icontains=q))
    if estado:
        incidentes = incidentes.filter(estado=estado)
    if area:
        incidentes = incidentes.filter(area_incidente=area)
    ...
    blanco_bold = Font(bold=True, color="FFFFFF")  # noqa: F841
    gris = PatternFill("solid", fgColor="F5F5F5")  # noqa: F841
    borde_fino = Border(
